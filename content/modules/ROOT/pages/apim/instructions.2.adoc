...
----
+
TIP: The *privateBaseURL* in this YAML is the Service endpoint of *Globex Mobile gateway*  servivc running on OpenShift. If interested, click to view the {openshift_cluster_console}/k8s/ns/globex-apim-{user_name}/services/globex-mobile-gateway[globex-mobile-gateway service URL^, window="_blank"]. The URL is diplayed under the Service routing section.
+
...
spec:
  ...
  deployment:
    apicastHosted:
      authentication:
        oidc:
          issuerType: "keycloak"
          issuerEndpoint: "https://client-manager:<REPLACE-WITH-CLIENT-CREDENTIALS>@sso.{openshift_subdomain}/realms/globex-{user-name}"
          authenticationFlow:
  ...
  backendUsages:
    globex-mobile-gateway-backend:
      path: /
----
. Replace the `<REPLACE-WITH-CLIENT-CREDENTIALS>` text in the YAML with the Client Credentials of the *client-manager* client you created in Keycloak in the previous step.
+
...
If you don't have this value, access Red Hat  {sso_tenant_console}/#/globex-{user_name}/clients[Keycloak Clients List^, window="sso"]. Login if needed with *({user_name}/{user_password})*. Click on the Client ID *client-manager*. You can copy the credentials from the *Credentials tab*
