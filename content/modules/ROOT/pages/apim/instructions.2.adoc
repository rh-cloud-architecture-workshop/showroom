...
. Navigate to OpenShift console {openshift_cluster_console}/topology/ns/globex-apim-{user_name}[OpenShift Console^, window="console"] and if needed login with *({user_name}/{user_password})*.
. Click on *(+)* button and then *Import Yaml* dropdown option on the top right corner of the OpenShift console
+
image::apim/import-yaml.png[] 
. Paste the following YAML content in the *Import YAML* text area, and click on the *Create* button
+
...
----
+
TIP: The *privateBaseURL* in this YAML is the Service endpoint of *Globex Mobile gateway*  servivc running on OpenShift. If interested, click to view the {openshift_cluster_console}/topology/ns/globex-apim-{user_name}/service/globex-mobile-gateway[globex-mobile-gateway service URL^, window="_blank"].
