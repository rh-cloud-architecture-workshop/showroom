...
[%collapsible]
====
. A https://docs.openshift.com/serverless/1.30/eventing/event-sinks/serverless-kafka-developer-sink.html[Kafka Sink for Apache Kafka, window="others"] helps in persisting an incoming CloudEvents over HTTP Kafka message (CloudEvent) to a Kafka topic. Event producers (such as apps, devices) can send to the Kafka Sinks there by reducing the complexity of new protocols and message formats for app developers. The Kafka Sinks then send the CloudEvents they receive to the configured Apache Kafka topic. 
