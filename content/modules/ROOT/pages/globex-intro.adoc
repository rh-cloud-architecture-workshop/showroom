...
* On your workstation, navigate to {openshift_cluster_console}[openshift_cluster_console, window="_console"]. Login with your username and password ({user_name}/{user_password}).
...
Click to navigate to https://streams-console-globex-mw-{user-name}.{openshift_subdomain}[AMQ streams console]. This redirects you to the AMQ streams console login page. For the purpose of this workshop, choose *Sign in with Anonymous Session* to access the console
+
...
* From the left-hand menu, navigate to *Kafka Clusters -> kafka -> Cluster overview*. You are provided with details about the Kafka cluster that has been setup for you with details including the number of brokers, the topics and also cluster metrics.
+
image::intro/amqstreams-cluster-overview.png[]
* Now choose the *Topics* menu on the left-hand side to see the list of topics. 
+
...
* Click on any of the messages listed to see the contents of the message. In this case, the body of each message consists of a JSON structure of a user activity event from the Globex web application.
+
